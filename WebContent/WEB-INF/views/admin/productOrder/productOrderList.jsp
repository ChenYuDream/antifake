		  ...
		  //弹出上传控件 
			...
			$("#edit_box_save").on("click",function(){
				if($("#edit_box_from input").val()&&$("#import_recipient").val()){
					$("#edit_box_from").attr("action",baseUrl+"/admin/productOrder/import").submit();
				}else{
...
<div class="row">
				<div class="span4 bs-docs-sidebar">
			        <ul class="nav nav-list bs-docs-sidenav affix">
			          <li class="active"><button class="btn btn-success" id="productOrder_btn">产品订单管理</button></li>
	...
	<form id="formSearch" class="form-search well" action="#" autocomplete="off">
		...
		<div>
		<label>订单号：</label> <input type="text" name="search_LIKE_soNo" class="input-medium" value="${param.search_LIKE_soNo}">
		<%-- <label>规格：</label> <input type="text" name="search_LIKE_productSize" class="input-medium" value="${param.search_LIKE_productSize}"> --%>
		<label>产品名称：</label> <input type="text" name="search_LIKE_product" class="input-medium" value="${param.search_LIKE_product}">
